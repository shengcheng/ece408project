...
// __global__ void unroll_x_kernel(float *X, float *X_unroll, dims x, dims w, dims y) {
// 	int c, p, q, row, col, filter_index, h_out, w_out, h_unroll, w_unroll, xoffset, uoffset;
// 	int index = blockDim.x * blockIdx.x + threadIdx.x;
// 	int n = blockIdx.y;
// 	int H_filter = w.dim[0];
// 	int W_filter = w.dim[1];
...
// 	int H_unroll = C * H_filter * W_filter;
//
// 	if (index < H_unroll * W_unroll) {
// 		row = index / W_unroll;
// 		col = index % W_unroll;
// 		h_out = col / W_out;
// 		w_out = col % W_out;
// 		c = row / (H_filter * W_filter);
// 		filter_index = row % (H_filter * W_filter);
// 		p = filter_index / W_filter;
// 		q = filter_index % W_filter;
// 		w_unroll = col;
// 		h_unroll = row;
// 		uoffset = (n * H_unroll + h_unroll) * W_unroll + w_unroll;
// 		xoffset = ((n * x.dim[1] + (h_out + p)) * x.dim[2] + (w_out + q)) * x.dim[3] + c;
...
__global__ void reroll_y_kernel(float *Y, float *Y_roll, dims y) {
	...
	if (index < y.dim[1] * y.dim[2] * y.dim[3]) {
		...
		Y_roll[yroll_offset] = (Y[y_offset] < 0) ? 0 : Y[y_offset];
	}
}
...
void average_pool_parallel(const float *x, float *y, const int xdims[4], const int ydims[4], const int pool_size) {
	...
	dim3 DimGrid(ydims[3], ydims[0], 1);
...
void conv_forward_unroll(const float *x, const float *w, float *y, const int xdims[4], const int wdims[4], const int ydims[4]) {
  ...
  // cudaMemcpyAsync(device_x + 0 * stride * x_size, x + 0 * stride * x_size, size_x / NUMSTREAM, cudaMemcpyHostToDevice, stream[0]);
	...
	// 	cudaMemcpyAsync(device_x + i * stride * x_size, x + i * stride * x_size, size_x / NUMSTREAM, cudaMemcpyHostToDevice, stream[i]);
	// 	unroll_x_kernel <<<DimGrid_unroll_x, DimBlock_unroll_x, 0, stream[i]>>> (device_x + i * stride * x_size, device_x_unroll + i * stride * x_unroll_size, x_d, w_d, y_d);
	// 	matrixMultiplyShared <<<DimGrid_matmul, DimBlock_matmul, 0, stream[i]>>> (device_w_unroll, device_x_unroll + i * stride * x_unroll_size, device_y_unroll + i * stride * y_unroll_size, numARows, numAColumns, numBRows, numBColumns, numCRows, numCColumns);
	// 	reroll_y_kernel <<<DimGrid_reroll_y, DimBlock_reroll_y, 0, stream[i]>>> (device_y_unroll + i * stride * y_unroll_size, device_y + i * stride * y_size, y_d);
	// 	cudaDeviceSynchronize();
	// }

	// unroll_x_kernel <<<DimGrid_unroll_x, DimBlock_unroll_x, 0, stream[15]>>> (device_x + 15 * stride * x_size, device_x_unroll + 15 * stride * x_unroll_size, x_d, w_d, y_d);
	// matrixMultiplyShared <<<DimGrid_matmul, DimBlock_matmul, 0, stream[15]>>> (device_w_unroll, device_x_unroll + 15 * stride * x_unroll_size, device_y_unroll + 15 * stride * y_unroll_size, numARows, numAColumns, numBRows, numBColumns, numCRows, numCColumns);
	// reroll_y_kernel <<<DimGrid_reroll_y, DimBlock_reroll_y, 0, stream[15]>>> (device_y_unroll + 15 * stride * y_unroll_size, device_y + 15 * stride * y_size, y_d);
