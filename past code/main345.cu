...
void conv_forward_unroll(const float *x, const float *w, const float *w2, float *pool, const int xdims[4], const int wdims[4], const int ydims[4], const int pooldims[4], const int w2dims[4], const int y2dims[4], const int pool2dims[4], int pool_size) {
	...
	cudaMemcpy(pool, device_pool_2, size_pool_2, cudaMemcpyDeviceToHost);
...
void fully_forward_parallel(float *x, float *w, float *w2, float *y, const int xdims[2], const int wdims[2], const int ydims[2], const int w2dims[2], const int y2dims[2]) {
  float *device_x, *device_w, *device_y, *device_w_2, *device_y_2;

  int size_x = sizeof(float) * xdims[0] * xdims[1];
  int size_w = sizeof(float) * wdims[0] * wdims[1];
  int size_y = sizeof(float) * ydims[0] * ydims[1];

	int size_w_2 = sizeof(float) * w2dims[0] * w2dims[1];
  int size_y_2 = sizeof(float) * y2dims[0] * y2dims[1];

	int numARows = xdims[0], numAColumns = xdims[1];
	int numBRows = wdims[0], numBColumns = wdims[1];
	int numCRows = ydims[0], numCColumns = ydims[1];

	int numARows2 = ydims[0], numAColumns2 = ydims[1];
	int numBRows2 = w2dims[0], numBColumns2 = w2dims[1];
	int numCRows2 = y2dims[0], numCColumns2 = y2dims[1];
  ...
  cudaMemcpy(device_w, w, size_w, cudaMemcpyHostToDevice);
	cudaMemcpy(device_w_2, w2, size_w_2, cudaMemcpyHostToDevice);
	...
	dim3 DimGrid_2(ceil((float)(numCColumns2) / TILE_WIDTH), ceil((float)(numCRows2) / TILE_WIDTH), 1);
	...
	relu_kernel <<<ceil((float)(ydims[0] * ydims[1]) / MAX_THREADS), MAX_THREADS>>> (device_y, ydims[0] * ydims[1]);
	matrixMultiplyShared <<<DimGrid_2, DimBlock_2>>> (device_y, device_w_2, device_y_2, numARows2, numAColumns2, numBRows2, numBColumns2, numCRows2, numCColumns2);

  cudaMemcpy(y, device_y_2, size_y_2, cudaMemcpyDeviceToHost);
...
void forward_operation(float *x, float *conv1, float *conv2, float *fc1,
  ...
  const int adims[] = {xdims[0], (xdims[1] - conv1dims[0] + 1), (xdims[2] - conv1dims[1] + 1), conv1dims[3]};
	...
	conv_forward_unroll(x, conv1, conv2, d, xdims, conv1dims, adims, bdims, conv2dims, cdims, ddims, pool_size);
  ...
  fully_forward_parallel(d, fc1, fc2, f, ddims2, fc1dims, edims, fc2dims, fdims);
